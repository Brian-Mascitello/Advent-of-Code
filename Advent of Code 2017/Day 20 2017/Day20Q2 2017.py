...
while measurements_to_make:
    ...
    # Find and remove collisions
    items_to_remove = list()
    for index in range(len(particle_list)):
        collision_position = particle_list[index][0]
        for item in particle_list[index + 1:]:
            if collision_position == item[0]:
                items_to_remove.append(particle_list[index])
                items_to_remove.append(item)
    for collisions in items_to_remove:
        if collisions in particle_list:
            particle_list.remove(collisions)
